...
class SignaturesTest {
  ...
  fun `signFlow returns signature of correct size`() = runBlockingTest {
    ...
    val (outFlow, signature) = privateKey.signFlow(
      certificate, LONG_DATA.asBufferedFlow(24)
    )
  ...
  fun `verifySignedFlow returns true for valid signatures`() = runBlockingTest {
    ...
    val (signFlow, deferredSig) = privateKey.signFlow(
      certificate,
      LONG_DATA.asBufferedFlow(24)
    )
    ...
    val outFlow2 = certificate.verifySignedFlow(
      LONG_DATA.asBufferedFlow(24),
      deferredSig.await()
    )
    assertThat(outFlow.flatten()).isEqualTo(LONG_DATA)
