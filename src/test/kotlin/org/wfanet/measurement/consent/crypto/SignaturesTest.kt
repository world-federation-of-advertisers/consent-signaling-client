...
import org.wfanet.measurement.consent.crypto.exception.DataFailsSignatureValidation
...
class SignaturesTest {
  ...
  fun `verifySignedFlow returns true for valid signatures`() = runBlockingTest {
    ...
    val outFlow = certificate.verifySignedFlow(LONG_DATA.asBufferedFlow(24), regularSig, BLOB_KEY)
    val outFlow2 =
      certificate.verifySignedFlow(LONG_DATA.asBufferedFlow(24), deferredSig.await(), BLOB_KEY)
    assertThat(outFlow.flatten()).isEqualTo(LONG_DATA)
  ...
  fun `verifySignedFlow throws for invalid signed Flow`() = runBlockingTest {
    ...
    val outFlow = certificate.verifySignedFlow(LONG_DATA.asBufferedFlow(24), signature, BLOB_KEY)
    assertFailsWith(DataFailsSignatureValidation::class) {
      assertThat(outFlow.flatten()).isEqualTo(LONG_DATA)
    }
    ...
    val outFlow2 =
      certificate.verifySignedFlow(LONG_DATA.asBufferedFlow(24), deferredSig.await(), BLOB_KEY)
    assertFailsWith(DataFailsSignatureValidation::class) {
      assertThat(outFlow2.flatten()).isEqualTo(DATA)
    }
  }
}
