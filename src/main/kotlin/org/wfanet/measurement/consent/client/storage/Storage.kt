 ...
 */
@Throws(DataFailsSignatureValidation::class)
suspend fun Blob.verifiedRead(
  ...
  return cert.verifySignedFlow(this.read(bufferSize), signature, blobKey)
}
...
suspend fun StorageClient.createSignedBlob(
  ...
  return this.createBlob(blobKey = blobKey, content = contentFlow) to deferredSig.await()
}
